...
LINKS = {
    "NoDescriptionLink":"https://docs.godotengine.org/{language}/stable/contributing/documentation/updating_the_class_reference.html",
    "GDEngineClass":"https://docs.godotengine.org/{language}/stable/classes/class_{gd_class}.html"
}
...
def main() -> None:
    ...
    for key in LINKS.keys():
        LINKS[key] = LINKS[key].replace("{language}", args.lang)
    ...
    for class_name, class_def in state.classes.items():
        # Prevent for generating documentation for @GlobalScope
    ...
    print("")
    #  Uncomment following lines to generate index file
...
def make_md_class(class_def: ClassDef, state: State, dry_run: bool, output_dir: str) -> None:
    ...
    with open(
        os.devnull if dry_run else os.path.join(output_dir, f"{class_name}.md"),
        "w",
        encoding="utf-8",
        newline="\n",
    ) as f:
        ...
        f.write(make_heading(class_name, 1, False,id="TOP"))
        ...
        if not has_description:
            ...
            s = s.replace(":ref:`","[")
            link = LINKS["NoDescriptionLink"]
            s = s.replace("<doc_updating_the_class_reference>`",f"]({link})")
            f.write(s)
        ...
        if len(class_def.properties) > 0:
            ...
            for property_def in class_def.properties.values():
                ...
                if default is not None and property_def.overrides:
                    ref = (
                        format_text_block(f"[member {property_def.overrides}.{property_def.name}]",property_def,state)
                    )
                    # Not using translate() for now as it breaks table formatting.
                    ml.append((type_md, property_def.name, f"{default} (overrides {ref})"))
                else:
                    ref = f"[{property_def.name}](#property_{property_def.name})"
        ...
        if len(class_def.constructors) > 0:
            f.write(make_heading("Constructors", 2))
        ...
        if len(class_def.methods) > 0:
            f.write(make_heading("Methods", 2))
        ...
        if len(class_def.operators) > 0:
            f.write(make_heading("Operators", 2))
        ...
        if len(class_def.theme_items) > 0:
            ...
            for theme_item_def in class_def.theme_items.values():
                ref = f"[{theme_item_def.name}](theme_{theme_item_def.data_name}_{theme_item_def.name})"
        ...
        if len(class_def.signals) > 0:
            ...
            for signal in class_def.signals.values():
                ...
                signal_anchor = f"signal_{signal.name}"
                #self_link = f"[{class_name}.{signal.name}](#{signal_anchor})"

                _, signature = make_method_signature(class_def, signal, "", state)
                f.write(make_heading(signature,4,id = signal_anchor))
                ...
                if signal.description is not None and signal.description.strip() != "":
                    ...
                    s = s.replace(":ref:`","[")
                    s = s.replace("<doc_updating_the_class_reference>`", "]("+LINKS["NoDescriptionLink"]+")")
                    f.write(s)
        ...
        if len(class_def.enums) > 0:
            ...
            for e in class_def.enums.values():
                ...
                enum_anchor = f"enum_{e.name}"
                #self_link = f"[{class_name}.{e.name}](#{enum_anchor})"

                if e.is_bitfield:
                    f.write(make_heading(f"flags **{e.name}**:",4,id=enum_anchor))
                else:
                    f.write(make_heading(f"enum **{e.name}**:",4,id=enum_anchor))

                for value in e.values.values():
                    # Also create signature and anchor point for each enum constant.
                    value_anchor = f"constant_{value.name}"

                    f.write(make_heading(f"{e.type_name.to_md(state)} **{value.name}** = ``{value.value}``",5,id=value_anchor))
        ...
        if len(class_def.constants) > 0:
            ...
            for constant in class_def.constants.values():
                ...
                constant_anchor = f"constant_{constant.name}"
                #self_link = f":ref:`🔗<{constant_anchor}>`"
                f.write(make_heading(f"**{constant.name}** = ``{constant.value}``",4,id=constant_anchor))
        ...
        if len(class_def.annotations) > 0:
            ...
            for method_list in class_def.annotations.values():  # type: ignore
                for i, m in enumerate(method_list):
                    ...
                    annotation_anchor = f"annotation_{m.name}"
                    ...
                    f.write(make_heading(signature,4,id = annotation_anchor))
        ...
        if any(not p.overrides for p in class_def.properties.values()) > 0:
            f.write(make_separator(True))
            f.write(make_heading("Property Descriptions", 2,id = "PropertyDescriptions"))
            ...
            for property_def in class_def.properties.values():
                ...
                property_anchor = f"property_{property_def.name}"
                ...
                f.write(
                    make_heading(f"{property_def.type_name.to_md(state)} **{property_def.name}**{property_default}",4,id = property_anchor)
                )
        ...
        if len(class_def.constructors) > 0:
            f.write(make_separator(True))
            f.write(make_heading("Constructor Descriptions", 2, id = "ConstructorDescriptions"))
            ...
            for method_list in class_def.constructors.values():
                for i, m in enumerate(method_list):
                    ...
                    constructor_anchor = f"constructor_{m.name}"
                    ...
                    f.write(make_heading(f"{ret_type} {signature}",4,id=constructor_anchor))
        ...
        if len(class_def.methods) > 0:
            f.write(make_separator(True))
            f.write(make_heading("Method Descriptions", 2,id = "MethodDescriptions"))
            ...
            for method_list in class_def.methods.values():
                for i, m in enumerate(method_list):
                    ...
                    #print("Method = \""+m.name+"\" ; i =",i)

                    if i == 0:
                        method_qualifier = ""
                        if m.name.startswith("_"):
                            method_qualifier = "private_"
                        method_anchor = f"{method_qualifier}method_{m.name}"
                        self_link = f" :ref:`🔗<{method_anchor}>`"

                    ret_type, signature = make_method_signature(class_def, m, "", state)

                    f.write(make_heading(f"{ret_type} {signature}",4,id=method_anchor))
        ...
        if len(class_def.operators) > 0:
            f.write(make_separator(True))
            f.write(".. md-class:: classref-descriptions-group\n\n")
            f.write(make_heading("Operator Descriptions", 2, id = "OperatorDescriptions"))
            ...
            for method_list in class_def.operators.values():
                for i, m in enumerate(method_list):
                    ...
                    operator_anchor = f"operator_{sanitize_operator_name(m.name, state)}"
                    for parameter in m.parameters:
                        operator_anchor += f"_{parameter.type_name.type_name}"
                    #self_link = f":ref:`🔗<{operator_anchor}>`"

                    ret_type, signature = make_method_signature(class_def, m, "", state)
                    f.write(make_heading(f"{ret_type} {signature}",4,id = operator_anchor))
        ...
        if len(class_def.theme_items) > 0:
            f.write(make_separator(True))
            f.write(make_heading("Theme Property Descriptions", 2, id = "ThemePropertyDescriptions"))
            ...
            for theme_item_def in class_def.theme_items.values():
                ...
                theme_item_anchor = f"theme_{theme_item_def.data_name}_{theme_item_def.name}"
                ...
                f.write(
                    make_heading(f"{theme_item_def.type_name.to_md(state)} **{theme_item_def.name}**{theme_item_default}",4,id=theme_item_anchor)
                )
        ...
        # Uncomment the next line to generate list of keys' definitions
        # f.write(make_footer())

        # Color tag support
        f.write(colors_tags_definitions(state))

def colors_tags_definitions(state:State) -> str:
    if len(state.used_colors) == 0:
        return ""
    out = "<style>\n"
...
def make_type(klass: str, state: State) -> str:
    ...
    def resolve_type(link_type: str) -> str:
        if link_type in state.classes and link_type != "@GlobalScope":
            return f"[{link_type}](./{link_type}.md)"
        else:
            #print_error(f'{state.current_class}.xml: Unresolved type "{link_type}".', state)
            link = LINKS["GDEngineClass"].format(gd_class = link_type.lower())
            return f"[{link_type}]({link})"

    if klass.endswith("[]"):  # Typed array, strip [] to link to contained type.
        link = LINKS["GDEngineClass"].format(gd_class="array")
        return f"[Array]({link})\\[{resolve_type(klass[:-len('[]')])}\\]"
    ...
    if klass.startswith("Dictionary["):  # Typed dictionary, split elements to link contained types.
        ...
        key = parts[0]
        value = parts[2]
        link = LINKS["GDEngineClass"].format(gd_class="dictionary")
        return f"[Dictionary]({link})\\[{resolve_type(key)}, {resolve_type(value)}\\]"
...
def make_enum(t: str, is_bitfield: bool, state: State) -> str:
    ...
    if c in state.classes and e in state.classes[c].enums and c != "@GlobalScope":
        if is_bitfield:
            if not state.classes[c].enums[e].is_bitfield:
                print_error(f'{state.current_class}.xml: Enum "{t}" is not bitfield.', state)
            return f"*bitfield*\\[[{e}](./{c}.md#enum_{e})\\]"
        else:
            return f"[{e}](./{c}.md#enum_{e})"

    link = LINKS["GDEngineClass"].format(gd_class = c.lower())
    return f"[{e}]({link}#enum_{c}_{e})"
...
def make_method_signature(
    ...
    Link = f"./{class_name}.md"

    if not class_name in state.classes or class_name == "@GlobalScope":
        link = LINKS["GDEngineClass"].format(gd_class = class_name)

    out = ""
    if isinstance(definition, MethodDef) and ref_type != "":
        if ref_type == "operator":
            op_name = definition.name.replace("<", "\\<")  # So operator "<" gets correctly displayed.
            out += f"[{op_name}]({Link}#{ref_type}_{sanitize_operator_name(definition.name, state)}"
            ...
            out += f"[{definition.name}]({Link}#{ref_type_qualifier}{ref_type}_{definition.name})"
        else:
            out += f"[{definition.name}]({Link}#{ref_type}_{definition.name})"
    else:
...
def make_heading(title: str, level: int, l10n: bool = True,id: str = "") -> str:
    if l10n:
        new_title = translate(title)
        if new_title != title:
            title = new_title
    if id == "":
        return "<a name=\""+title+"\"></a>\n\n"+level*'#'+" "+title+"\n\n"
    return "<a name=\""+id+"\"></a>\n\n"+level*'#'+" "+title+"\n\n"


def make_footer() -> str:
    # Generate reusable abbreviation substitutions.
    # This way, we avoid bloating the generated MD with duplicate abbreviations.
...
def make_link(url: str, title: str) -> str:
    ...
    if match:
        ...
        if match.lastindex == 2:
            ...
            return f"[{groups[1]}](../{groups[0]}.html{groups[1]}../{groups[0]})"
...
def format_text_block(
    ...
    text = escape_md(text, next_brac_pos)
    ...
    while True:
        ...
        # Tag is a cross-reference or a formatting directive.
            ...
            # Cross-references to items in this or other class documentation pages.
                ...
                if link_target == "":
                    ...
                    if (
                        ...
                        ref_type = "{}".format(tag_state.name)

                        link = "./{gd_class}.md#{anchor}"

                        if target_class_name in state.classes and target_class_name != "@GlobalScope":
                            class_def = state.classes[target_class_name]

                            if tag_state.name == "method":
                                if target_name.startswith("_"):
                                    ref_type = "private_method"
                            ...
                            # This class is from godot engine
                            link = LINKS["GDEngineClass"]+"#class_{gd_class}_{anchor}"
                            # print_error(
                        ...
                        link = link.format(gd_class = target_class_name.lower(),anchor = f"{ref_type}_{target_name}")
                        tag_text = f"[{repl_text}]({link})"
                        escape_pre = True
                        escape_post = True
            ...
            # Formatting directives.
                ...
                # Make a new paragraph instead of a linebreak, md is not so linebreak friendly
                tag_text = "\n\n"
            ...
            elif tag_state.name == "color":
                if tag_state.closing:
                    tag_depth -= 1
                    escape_post = True
                    try:
                        tag_text = f"</{last_color.pop()}>"
                    except:
                        print_error(f'closing [/color] tag occured before opening one.',state)
                        tag_text = ""
                else:
                    tag_depth += 1
                    escape_pre = True
                    last_color.append(tag_state.arguments)
                    tag_text = f"<{tag_state.arguments}>"
                    state.used_colors.add(tag_state.arguments)
                ...
                if tag_state.closing:
                    ...
                    # Tag probably is a reference to an engine class
...
def escape_md(text: str, until_pos: int = -1) -> str:
    # Escape \ character, otherwise it ends up as an escape character in md
    pos = 0
    while True:
